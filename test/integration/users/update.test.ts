import * as chai from 'chai';
import chaiHttp = require('chai-http');
import { getRepository } from 'typeorm';
import { UserEntity } from '../../../src/models/UserEntity';
import { app } from '../../../src';
import { OK, UNPROCESSABLE_ENTITY } from 'http-status-codes'

chai.use(chaiHttp);
const { expect } = chai;

describe('Endpoint test for users: PUT /users', () => {
    const USERS_ENDPOINT = '/api/users';
    const LOGIN_ENDPOINT = '/api/users/login';

    it('GIVEN a logged in user, ' +
        'WHEN updating their password and bio, ' +
        'THEN a Response with HTTP STATUS OK ' +
        'and a successful login with new password',
        async () => {

            // Arrange

            const userRepo = getRepository(UserEntity);
            await userRepo.insert({
                username: 'John F Doe',
                email: 'sample.user@mydomain.com',
                bio: '',
                image: '',
                
                // precomputed salt and hash for password 'Hp6V.M/5$mE:$?]H'
                salt: '21ac654d8e3a8d75ab53a2d57005a602',
                hash: '',
            });

            const oldLogin = {
                user: {
                    email: 'sample.user@mydomain.com',
                    password: 'Hp6V.M/5$mE:$?]H',
                },
            };

            const update = {
                user: {
                    password: '12345',
                    bio: 'lorem ipsum sample test debug',
                },
            };

            const newLogin = {
                user: {
                    email: 'sample.user@mydomain.com',
                    password: '12345'
                },
            };

            // Act

            const oldLoginResult = await chai
                .request(app)
                .post(LOGIN_ENDPOINT)
                .send(oldLogin);
            const token = oldLoginResult.body.user.token;
            const changePasswordResult = await chai
                .request(app)
                .put(USERS_ENDPOINT)
                .auth(token, { type: 'bearer' })
                .send(update);
            const newLoginResult = await chai
                .request(app)
                .post(LOGIN_ENDPOINT)
                .send(newLogin);

            // Assert

            expect(changePasswordResult).to.have.status(OK);
            expect(changePasswordResult.body?.user).to.include({
                email: 'sample.user@mydomain.com',
                username: 'John F Doe',
                bio: 'lorem ipsum sample test debug',
                image: '',
            });
            expect(changePasswordResult.body?.user?.token).to.be.a('string');
            expect(newLoginResult).to.have.status(OK);
        });
})
